...
REM Usar PowerShell para entrada más robusta
powershell -Command "$choice = Read-Host 'Poblar base de datos? [S/N]'; if ($choice -eq '') { $choice = 'N' }; Write-Output $choice" > temp_choice.txt
set /p POPULATE_DB=<temp_choice.txt
del temp_choice.txt

if /i "%POPULATE_DB%"=="s" goto do_populate
if /i "%POPULATE_DB%"=="si" goto do_populate
if /i "%POPULATE_DB%"=="y" goto do_populate
if /i "%POPULATE_DB%"=="yes" goto do_populate
goto skip_populate
