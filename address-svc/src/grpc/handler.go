...
func dtoToCreate(a *pb.Address) dto.AddressCreate {
...
func modelToProto(m *model.Address) *pb.Address {
	return &pb.Address{
		Id:      m.ID,
		Floor:   m.Floor,
		Unit:    m.Unit,
...
func (h *Handler) Create(ctx context.Context, req *pb.CreateAddressRequest) (*pb.CreateAddressResponse, error) {
	addr, err := h.svc.Create(dtoToCreate(req.GetAddress()))
	if err != nil {
		return nil, err
	}
	return &pb.CreateAddressResponse{Address: modelToProto(addr)}, nil
}

func (h *Handler) Get(ctx context.Context, req *pb.GetAddressRequest) (*pb.GetAddressResponse, error) {
	addr, err := h.svc.Get(req.GetId())
	if err != nil {
		return nil, err
	}
	return &pb.GetAddressResponse{Address: modelToProto(addr)}, nil
}
